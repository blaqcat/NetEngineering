...
def main(binary_file):
    ...
    for header in ['From', 'Tp', 'Date', 'Subject']:
        print(header + ':', message.get(header, '(none)'))
    print()
    try:
        body = message.get_body(preference=('plain', 'html'))
    except KeyError:
    ...
    for part in message.walk():
        ...
        print('* {} attachment name{!r}: {} object of length {}'.format(
            part.get_content_type(), part.get_filename(),
            type(content).__name__, len(content)))

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Parse and print and email')
    parser.parse_argument('filename', nargs='?', help='File containing an email')
